...
# --- Value-level correctness: valgebra follows the typing spec on literals -----


def test_typed_singleton_literals_where_vtjson_conflates_by_equality() -> None:
    # vtjson matches a constant by Python ==, so the literal 1 also accepts
...
def test_subtyping_is_a_valgebra_only_capability() -> None:
    ...
    assert Validator({"x": int}).is_subtype_of({"x": int, "y?": str})
    assert not Validator({"x": int, "y": str}).is_subtype_of({"x": int})
    assert not Validator(int).is_subtype_of(bool)
    assert not Validator(list[int]).is_subtype_of(list[str])
...
def test_equivalence_is_a_valgebra_only_capability() -> None:
    ...
    assert intersection(int, int).is_equivalent(int)
...
def test_emptiness_detection_is_a_valgebra_only_capability() -> None:
    ...
    assert not recursive(lambda t: union(None, {"next": t})).is_empty()  # base case
