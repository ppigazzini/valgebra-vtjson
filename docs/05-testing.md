...
collected rather than asserted one at a time. The surfaces swept so far are
container literals, constructs at construction and at validation time, string
formats, classes that declare keys, compound schemas, typing forms, callable
forms, value implementations, and the interpreter.
